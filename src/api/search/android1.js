...
module.exports = function(app) {
    async function fetchContent(query) {
        try {
            ...
            const response = await axios.get(`https://api.rynn-archive.biz.id/search/android1?q=${encodeURIComponent(query)}`);
            return response.data;
        } catch (error) {
            console.error("Error fetching content from NekoApi:", error);
            throw error;
        }
    }
